...
//Output lower 8-bits of u8_c to LCD data lines
void outputTo8BusLCD(uint8_t u8_c) {
  LCD0O = u8_c & 0x01;          //D0
  LCD1O = (u8_c >> 1)& 0x01;    //D1
  LCD2O = (u8_c >> 2)& 0x01;    //D2
  LCD3O = (u8_c >> 3)& 0x01;    //D3
  LCD4O = (u8_c >> 4)& 0x01;    //D4
  LCD5O = (u8_c >> 5)& 0x01;    //D5
  LCD6O = (u8_c >> 6)& 0x01;    //D6
  LCD7O = (u8_c >> 7)& 0x01;    //D7
}
...
//**********************************************************
// Create the following macros to allow the LCD service task to be more
// ef?cient, more readable, and implement the full Hitachi 44780 command set
//**********************************************************
/*
ESOS_TASK_WAIT_LCD_WRITE_COMMAND( pst_lcdStruct, u8_cmd ) // RS=0; D= 0-0x3F
ESOS_TASK_WAIT_LCD_SET_CG_ADDRESS( pst_lcdStruct, u8_addr ) // RS=0; D=0x40-0x7F
ESOS_TASK_WAIT_LCD_SET_DATA_ADDRESS( pst_lcdStruct, u8_addr ) // RS=0; D=0x80-0xFF
ESOS_TASK_WAIT_LCD_WHILE_BUSY( pst_lcdStruct ) // RS=0
ESOS_TASK_WAIT_LCD_READ_ADDRESS( pst_lcdStruct, u8_addr ) // RS=0
ESOS_TASK_WAIT_LCD_WRITE_DATA( pst_lcdStruct, u8_data ) // RS=1
ESOS_TASK_WAIT_LCD_READ_DATA( pst_lcdStruct, u8_addr ) // RS=
*/
